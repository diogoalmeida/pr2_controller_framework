...
namespace cartesian_controllers {
  ...
  sensor_msgs::JointState MechanismIdentificationController::updateControl(const sensor_msgs::JointState &current_state, ros::Duration dt)
  {
    ...
    if (!has_initial_)
    {
      ...
      adaptive_controller_.initEstimates(translational_dof_ground_, rotational_dof_ground_); // Initialize with ground truth for now TODO: add wrong initial estimate
      elapsed_ = ros::Time(0);
      has_initial_ = true;
    }
